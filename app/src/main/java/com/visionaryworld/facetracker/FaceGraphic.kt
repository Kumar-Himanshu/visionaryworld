 ...
 */
class FaceGraphic internal constructor(overlay: FaceGraphicOverlay) : FaceGraphicOverlay.Graphic(overlay) {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        val x: Float = translateX(face.position.x + face.width / 2)
        val y: Float = translateY(face.position.y + face.height / 2)
        ...
        val xOffset: Float = scaleX(face.width / 2.0f)
        val yOffset: Float = scaleY(face.height / 2.0f)
        val left = x - xOffset
        val top = y - yOffset
        val right = x + xOffset
